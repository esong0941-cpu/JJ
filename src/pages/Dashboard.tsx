...
export function Dashboard({ market, lastUpdate, topShorts, onStockClick, onNavigate }: Props) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { icon: Activity, label: '단타 추천 종목', count: 8, tab: 'short', color: 'text-orange-400', bg: 'bg-orange-900/20 border-orange-800/50' },
          { icon: TrendingUp, label: '장타 추천 종목', count: 8, tab: 'long', color: 'text-green-400', bg: 'bg-green-900/20 border-green-800/50' },
          { icon: BarChart2, label: '기업분석 대상', count: 5, tab: 'analysis', color: 'text-blue-400', bg: 'bg-blue-900/20 border-blue-800/50' },
        ].map(item => (
          <button
            key={item.tab}
